...
async function getNpmConfig() {
  const npmcliConfig = new NpmcliConfig({
    definitions: {},
    npmPath: path$a.dirname(process.cwd()),
...
function parseDependencies(pkg, type, shouldUpdate) {
  return Object.entries(pkg[type] || {}).map(([name, version]) => parseDependency(name, version, type, shouldUpdate));
}
function parseDependency(name, version, type, shouldUpdate) {
  return {
    name,
...
export { CheckPackages as C, DependenciesTypeShortMap as D, _debug as _, resolveDependencies as a, resolveDependency as b, loadPackages as c, dumpDependencies as d, DiffMap as e, getVersionOfRange as f, getDefaultExportFromCjs as g, getPrefixedVersion as h, getPackageData as i, createDependenciesFilter as j, loadPackage as l, parseDependencies as p, resolvePackage as r, toArray as t, updateTargetVersion as u, writePackage as w };
