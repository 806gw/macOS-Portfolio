...
import { g as getDefaultExportFromCjs, _ as _debug, t as toArray, e as DiffMap, D as DependenciesTypeShortMap, u as updateTargetVersion, f as getVersionOfRange, h as getPrefixedVersion, C as CheckPackages, w as writePackage, c as loadPackages, i as getPackageData, r as resolvePackage, j as createDependenciesFilter, d as dumpDependencies } from './shared/taze.e18d72a9.mjs';
...
var stringWidthExports = stringWidth$1.exports;
...
const _EventEmitter$1 = require$$0$2;
...
const _EventEmitter = require$$0$2;
...
async function resolveConfig(options) {
  ...
  return deepmerge$1(configOptions, options);
}
...
function renderChange(change, interactive) {
  ...
  return [
    ...
    change.latestVersionAvailable && semver.minVersion(change.targetVersion).toString() !== change.latestVersionAvailable ? c.dim(c.magenta(`(${change.latestVersionAvailable} available)`)) : ""
  ];
}
function renderChanges(pkg, options, interactive) {
  ...
  if (changes.length) {
    ...
    const diffEntries = Object.keys(diffCounts).length ? Object.entries(diffCounts).map(([key, value]) => `${c.yellow(value)} ${key}`).join(", ") : c.dim("no change");
    lines.push(
...
async function loadGlobalPackage(options) {
  ...
  const deps = Object.entries(npmOut.dependencies).filter(([_name, i]) => i?.version).map(
    ([name, i]) => ({
      name,
